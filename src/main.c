...
customStatus_e customProcessor(txContext_t *context) {
    if ((context->currentField == TX_RLP_DATA) &&
        ...
        if (context->currentFieldPos == 0) {
            ...
            if (os_memcmp(context->workBuffer, VOTE_ID, 4) == 0) {
                voteProvisioned = true;
            }

            //voteProvisioned = os_memcmp(context->workBuffer, VOTE_ID, 4) == 0
        }
        if (tokenProvisioned || voteProvisioned) {
...
#if defined(TARGET_BLUE)
    snprintf(strings.common.fullAddress, sizeof(strings.common.fullAddress), "t0%.*s", 40, tmpCtx.publicKeyContext.address);
...
void finalizeParsing(bool direct) {
  ...
  if (tmpContent.txContent.destinationLength != 0) {
    ...
    */
    ...
    strings.common.fullAddress[0] = 't';
    strings.common.fullAddress[1] = '0';
  ...
  if (voteProvisioned)
    strcpy(strings.common.fullAmount, "Vote");
  {
    /* code */
  }
  // Compute maximum fee
